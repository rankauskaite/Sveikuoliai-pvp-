...
class _GoalPageState extends State<GoalScreen> {
  ...
  Future<void> _fetchSharedGoalData() async {
  try {
    SharedGoalModel? fetchedSharedGoal =
        await _sharedGoalService.getSharedGoalEntry(widget.goal.sharedGoalId);
    if (fetchedSharedGoal != null) {
      setState(() {
        sharedGoal = fetchedSharedGoal;
      });
    } else {
      throw Exception("Gautas `null` shared goal objektas");
    }
  } catch (e) {
    showCustomSnackBar(context, 'Klaida gaunant shared goal duomenis ❌', false);
  }
}

Future<void> _fetchSharedGoalTasks() async {
  try {
    List<SharedGoalTask> tasks =
        await _sharedGoalTaskService.getSharedGoalTasks(widget.goal.sharedGoalModel.id);

    setState(() {
      sharedGoalTasks = tasks;
    });
  } catch (e) {
    showCustomSnackBar(context, 'Klaida kraunant shared goal užduotis ❌', false);
  }
}
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                    height: 200,
                    child: sharedGoalTasks.isEmpty
                        ? const Text("Nėra progreso duomenų")
                        : _buildSharedGoalProgressChart(),
                    ),
                  ],
                ),
              ),
